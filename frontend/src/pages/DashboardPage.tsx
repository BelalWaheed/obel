...
import { ListTodo, Timer, CheckCircle2, TrendingUp, Coffee, Zap } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const currentCaffeine = useCoffeeStore((s) => s.getCurrentCaffeineLevel)
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const completedToday = useMemo(() => getCompletedToday(), [getCompletedToday, tasks])
  ...
  const stats = [
    ...
    { label: "Today's Coffee", value: getCupsToday(), icon: Coffee, color: 'text-orange-600', bgColor: 'bg-orange-600/10' },
    { label: 'Caffeine Impact', value: `${currentCaffeine()}%`, icon: Zap, color: 'text-yellow-500', bgColor: 'bg-yellow-500/10' },
  ]
